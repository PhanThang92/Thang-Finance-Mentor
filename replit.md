...
- 10 navigation sections (sidebar grouped):
  ...
  - **Hệ sinh thái**: Sản phẩm (ProductsPanel – card list + tabbed editor: basic info, page content, pricing/CTA, SEO), Leads (LeadsPanel – full-height: header+subtitle, "Xuất dữ liệu" CSV export, status tab strip with counts, filter bar with keyword+nguồn+sản phẩm, full table with 8 columns including inline status dropdown, right detail panel with contact/source/status/notes, empty states)
