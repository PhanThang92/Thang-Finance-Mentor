...
/* ── Article format template (HTML) ──────────────────────────────── */
const ARTICLE_TEMPLATE = `<h2>Mở đầu — Vấn đề hoặc câu hỏi chính</h2>
<p>Viết 1–2 đoạn ngắn để đặt vấn đề. Tại sao bài viết này quan trọng với người đọc? Điều gì khiến họ muốn đọc tiếp?</p>
<h2>Phần 1 — Tên tiêu đề rõ ràng, cụ thể</h2>
<p>Nội dung phần này. Mỗi đoạn văn chỉ nên có 3–5 câu. Xuống dòng thường xuyên để dễ đọc trên điện thoại.</p>
<blockquote><p>Insight hoặc câu trích dẫn quan trọng nhất của phần này — điều người đọc sẽ nhớ mãi.</p></blockquote>
<h2>Phần 2 — Tên tiêu đề rõ ràng, cụ thể</h2>
<p>Nội dung phần này...</p>
<h3>Tiểu mục (khi cần đi sâu hơn vào một ý)</h3>
<p>Chi tiết của tiểu mục...</p>
<ul>
  <li>Điểm quan trọng 1</li>
  <li>Điểm quan trọng 2</li>
  <li>Điểm quan trọng 3</li>
</ul>
<h2>Kết luận — Điều bạn nên làm ngay bây giờ</h2>
<p>Tóm tắt bài viết trong 2–3 câu. Đưa ra hành động cụ thể cho người đọc. Kết thúc bằng câu gợi mở hoặc lời mời tiếp tục hành trình.</p>`;
...
export function ArticlesPanel({ adminKey }: { adminKey: string }) {
  ...
  const generateOg = useCallback(async () => {
    ...
    try {
      const result = await adminApi.generateArticleOg(adminKey, form.id);
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
        ...
        <div style={s.card}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.875rem" }}>
            <h3 style={{ fontSize: "11px", fontWeight: 700, letterSpacing: "0.10em", textTransform: "uppercase", color: A.textMuted, margin: 0 }}>Nội dung</h3>
            <div style={{ display: "flex", gap: "2px", background: "#f3f4f6", borderRadius: "7px", padding: "2px" }}>
              {(["write", "preview"] as const).map((tab) => (
                <button
                  ...
                  style={{
                    padding: "4px 14px", borderRadius: "5px", border: "none", cursor: "pointer",
                    fontSize: "11.5px", fontWeight: 600,
                    background: contentTab === tab ? "#fff" : "transparent",
                    color: contentTab === tab ? A.primary : A.textMuted,
                    boxShadow: contentTab === tab ? "0 1px 3px rgba(0,0,0,0.10)" : "none",
                    transition: "all 0.15s ease",
                  }}
                >
                  {tab === "write" ? "Soạn thảo" : "Xem trước"}
                </button>
              ))}
            </div>
          </div>
              ...
              <div style={{ borderTop: `1px solid ${A.border}`, paddingTop: "0.875rem" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.75rem" }}>
                  <div>
                    ...
                    <p style={{ fontSize: "11.5px", color: A.textMuted, margin: "3px 0 0" }}>Tạo ảnh thương hiệu 1200×630 từ tiêu đề bài viết.</p>
                  </div>
                  {form.id ? (
                    <button
                      type="button"
                    ...
                    <span style={{ fontSize: "11.5px", color: A.textMuted, fontStyle: "italic" }}>Lưu bài viết trước khi tạo ảnh.</span>
                  )}
                </div>
