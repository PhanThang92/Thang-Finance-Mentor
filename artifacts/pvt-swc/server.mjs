...
async function fetchMeta(urlPath, origin) {
  ...
  /* News article: /tin-tuc/<catSlug>/<postSlug> */
...
const server = http.createServer(async (req, res) => {
  ...
  /* 1. Try to serve static asset from dist/public */
  if (urlPath !== "/" && path.extname(urlPath)) {
    const filePath = path.join(DIST_DIR, urlPath);
    if (serveStatic(res, filePath)) return;
  }

  /* 2. Load index.html template */
  ...
  const origin = `https://${req.headers.host ?? "phanvanthang.replit.app"}`;

  /* 3. Bot → inject page-specific OG meta */
  if (isBot(ua)) {
    try {
  ...
  /* 4. Regular browser → serve SPA shell */
